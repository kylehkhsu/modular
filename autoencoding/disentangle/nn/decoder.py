    ...
    def __init__(self, *, latent_size, normalize_outputs, cond_size=256, key):
        ...
        self.up_blocks = (
            ...
            (
                ...
                ResCondConvBlock(cond_size, 64, key=next(keys)),
                Upsample(64, out_channels=32, key=next(keys)),
                eqx.nn.GroupNorm(32, channelwise_affine=False) if normalize_outputs else eqx.nn.Identity()
            ),
            (
                ResCondConvBlock(cond_size, 32, key=next(keys)),
                ResCondConvBlock(cond_size, 32, key=next(keys)),
                Upsample(32, out_channels=3, key=next(keys)),
                eqx.nn.Identity()
            ),
        )
