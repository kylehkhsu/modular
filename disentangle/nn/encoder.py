...
class ResidualEncoder(eqx.Module):
    ...
    def __init__(self, *, latent_size, key):
        ...
        self.down_blocks = (
            (
                ResConvBlock(3, out_channels=16, key=next(keys)),
                ResConvBlock(16, key=next(keys)),
                Downsample(16, out_channels=32, key=next(keys)),
            ),
            (
        ...
        self.dense = eqx.nn.Sequential(
            [
                eqx.nn.Lambda(lambda x: jnp.mean(x, axis=(1, 2))),
